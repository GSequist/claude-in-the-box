// scans TCP ports on the 127.0.0.1 and launches socat process for every port
...
// getEth0IP dynamically detects the IPv4 address assigned to eth0.
// This is needed because the VM IP is assigned at boot time by Firecracker.
...
func (f *Forwarder) startPortForwarding(ctx context.Context, p *PortToForward) {
	...
	// socat -d -d TCP4-LISTEN:4000,bind=10.0.1.x,fork TCP4:127.0.0.1:4000
	// reuseaddr is used to fix the "Address already in use" error when restarting socat quickly.

	// Dynamically detect eth0 IP at forwarding time (not init time)
	// This ensures the IP is available after network is fully up
	bindIP := f.sourceIP
	if eth0IP := getEth0IP(); eth0IP != nil {
		bindIP = eth0IP
		f.logger.Debug().
			IPAddr("eth0IP", eth0IP).
			Msg("Using dynamically detected eth0 IP for socat binding")
	} else {
		f.logger.Warn().
			IPAddr("fallbackIP", f.sourceIP).
			Msg("Could not detect eth0 IP, using fallback")
	}

	cmd := exec.CommandContext(ctx,
		"socat", "-d", "-d", "-d",
		fmt.Sprintf("TCP4-LISTEN:%v,bind=%s,reuseaddr,fork", p.port, bindIP.To4()),
		fmt.Sprintf("TCP%d:127.0.0.1:%v", p.family, p.port),
	)
